...
The process is actually really similar to the process we used to create an R script. Start by clicking on the icon shown below in @fig-new_quarto_document_01.
...
The YAML header is unlike anything we've seen before. The YAML header always begins and ends with dash-dash-dash (`---`) typed on its own line (1 & 6 in @fig-new-quarto-document-o4). The code written inside the YAML header generally falls into two categories:
