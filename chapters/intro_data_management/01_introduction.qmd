...
At this point in the book, you've already been exposed to several of the most important functions in the `dplyr` package. You saw the `filter()` function in the [Speaking R's language](#packages) chapter, the `mutate()` function in the [chapter on exporting data](#coerce-a-numeric-variable), and the `summarise()` function all over the descriptive analysis part of the book. However, we mostly glossed over the details at those points. In this section, we want to dive just a tiny bit deeper into how the `dplyr` functions work -- but not too deep.
